...
function extractListingUrls(html: string): Array<{ url: string; title: string; image: string; excerpt: string }> {
  ...
  const containers = $('article, .post, .entry, .td-block-span6, .td_module_flex').toArray();
  ...
  for (const el of containers) {
    ...
    const anchor =
      $el.find('h2.entry-title a, h1.entry-title a, .entry-title a, .td-module-title a, h2 a, h3 a').first();
    const url = toAbsoluteUrl(anchor.attr('href') ?? '', BASE_URL);
    const title = anchor.text().trim();
    if (!url || !title) continue;

    const image = toAbsoluteUrl(
      $el.find('.wp-post-image, .td-module-image img, .entry-thumbnail img, figure img').first().attr('src') ?? '',
      BASE_URL,
    );

    const excerpt = $el.find('.entry-summary, .td-excerpt, .entry-content p, .excerpt').first().text().trim();
...
function hasNextPage(html: string): boolean {
  const $ = cheerio.load(html);
  return $('a.next.page-numbers, .td-icon-menu-right, a[rel="next"], .nav-next a').length > 0;
}
...
async function scrapeDetail(url: string): Promise<Partial<RawOpportunity>> {
  ...
  const contentEl = $('.entry-content, .td-post-content, .post-content, article .content').first();
