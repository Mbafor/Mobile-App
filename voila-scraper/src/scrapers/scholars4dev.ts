...
function extractListingUrls(html: string): Array<{ url: string; title: string; image: string; excerpt: string }> {
  ...
  for (const el of containers) {
    ...
    const image = toAbsoluteUrl(
      $el.find('.wp-post-image, .entry-thumbnail img, figure img, img.attachment-thumbnail').first().attr('src') ?? '',
      BASE_URL,
    );

    const excerpt = $el.find('.entry-summary, .entry-content p, .post-excerpt').first().text().trim();
...
function hasNextPage(html: string): boolean {
  const $ = cheerio.load(html);
  return $('a.next.page-numbers, .next a, a[rel="next"], .navigation .next').length > 0;
}
...
async function scrapeDetail(url: string): Promise<Partial<RawOpportunity>> {
  ...
  const contentEl = $('.entry-content, .post-content, .post-body, article .content').first();
